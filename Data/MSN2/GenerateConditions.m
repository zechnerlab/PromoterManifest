function conditions = GenerateConditions()
%% DM case
...
%% FM4 case - this one has variable intervals
numPulse = 4;
tInterval = [7.5, 10, 15, 20];
nameInterval = {'75', '10', '15', '20'};

for i=1:length(tInterval)
    
    for k=1:length(conc)
        condition.PulseParameters = [numPulse, time, tInterval(i)];
        condition.Concentration = conc(k);
        
        if (condition.Concentration < 1000)
            condition.Name = ['FM4_' nameInterval{i} 'minINT_' num2str(conc(k)) 'nM'];
        else
            condition.Name = ['FM4_' nameInterval{i} 'minINT_' num2str(floor(conc(k)/1000)) 'uM'];
        end
        conditions{counter} = condition;
        counter = counter + 1;
    end
