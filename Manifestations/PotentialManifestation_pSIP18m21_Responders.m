...
feature4 = 'maxTr';
feature5 = 'pActivate';
feature6 = 'varTrOutputR';

sem1 = 'meanTrOutputRStd';
sem2 = 'meanTrOutputStd';
sem3 = 'meanTauAStd';
sem4 = 'maxTrStd';
...
for k=1:length(concentrations)
    ...
    [data6, promVec, conditionMat] = GetDataMatrix(dataB, promoters{l}, concentrations(k), [1], [], [-1], feature6);
    ...
    subplot(3, length(concentrations), k);
    
    p = bar(durations, data3, 0.7); hold on;
    
    binofit(numResponders, numCells);
    set(p, 'FaceColor', [0.6, 0.6, 0.8]);
    p = errorbar(durations, data3, data3sem*semFactor, 'ro'); hold on;
    ...
    subplot(3, length(concentrations), k + length(concentrations));
    ...
    p = errorbar(durations, data1, data1sem*semFactor, 'bo-');
    set(p, 'MarkerFaceColor', 'b');
    set(p, 'MarkerEdgeColor', 'b');
    
    subplot(3, length(concentrations), k + 2*length(concentrations));
