...
for i=1:length(kIdx)
    ...
    for l=1:length(cIdx)
        ...
        responders = and(results.Promoters{k}.PActivated>0.5, results.Promoters{k}.Valid);
        ...
        figure(41);
        subplot(length(cIdx), 2, (condIdx-1)*length(kIdx) + i);
        for j=1:length(results.Promoters{k}.YFP)
            
           if (results.Promoters{k}.Valid(j)==0)
               continue;
           end
           ...
           plot(results.Promoters{k}.YFP{j}.MeasurementTime/60, results.Promoters{k}.YFP{j}.Measurement, col); hold on;
        end
...
%         figure(3);
%         %numCells = 5;
%         respIdx = find(responders);
%         %numCells = length(respIdx);
%         for j=5:10
%             subplot(10, 2, (condIdx-1)*2 + 1);
%             meanTr = results.Promoters{k}.MeanTr(respIdx(j), :);
%             t = results.Promoters{k}.tGrid;
%             
%             p = stairs(t/60, meanTr);hold on;
%             %set(p, 'Color', [0.1, 0.1, 0.1, 0.1]);
%             
%             subplot(10, 2, (condIdx-1)*2 + 2);
%             p = plot(results.Promoters{k}.YFP{respIdx(j)}.MeasurementTime/60, results.Promoters{k}.YFP{respIdx(j)}.Measurement, '-');
%             %set(p, 'Color', [0.1, 0.1, 0.1, 0.1]);
%             hold on;
%             
%         end
