function [Mat, promVec, conditionMat, SEM, Runs] = GetDataMatrix(dataB, promoter, conc, numPulses, duration, interval, features)
...
if (isfield(dataB, 'DataSEM'))
   SEM = dataB.DataSEM(selIdx, featureIdx); 
else
   SEM = nan*Mat; 
end

if (isfield(dataB, 'DataCube'))
    Runs = dataB.DataCube(selIdx, featureIdx, :);
else
    Runs = [];
end
end